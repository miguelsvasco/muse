...
def log_marginal_image_estimate(recon_image, image, z, mu, logvar, batch_size, n_samples):
...
def log_marginal_label_estimate(recon_label, label, z, mu, logvar, batch_size, n_samples):

    log_p_x_given_z_2d = categorical_log_pdf(x=label, mu=recon_label)
    log_q_z_given_x_2d = gaussian_log_pdf(z, mu, logvar)
    log_p_z_2d = unit_gaussian_log_pdf(z)
...
def log_joint_estimate(recon_mnist, mnist, recon_svhn, svhn, recon_label, label, z, mu, logvar, batch_size, n_samples):

    # Image
    log_p_x_mnist_given_z_img_2d = -torch.sum(F.mse_loss(input=recon_mnist.view(batch_size * n_samples, -1),
                                     target=mnist.view(batch_size * n_samples, -1),
                                     reduction='none'), dim=-1)

    log_p_x_svhn_given_z_img_2d = -torch.sum(F.mse_loss(input=recon_svhn.view(batch_size * n_samples, -1),
                                                         target=svhn.view(batch_size * n_samples, -1),
                                                         reduction='none'), dim=-1)

    # Label
    log_p_x_label_given_z_label_2d = categorical_log_pdf(x=label, mu=recon_label)

    log_q_z_top_given_x_label_2d = gaussian_log_pdf(z, mu, logvar)
    log_p_z_top_2d = unit_gaussian_log_pdf(z)

    log_weight_2d = log_p_x_mnist_given_z_img_2d + log_p_x_svhn_given_z_img_2d + log_p_x_label_given_z_label_2d + log_p_z_top_2d - log_q_z_top_given_x_label_2d
    log_weight = log_weight_2d.view(batch_size, n_samples)
...
def log_conditional_label_estimate(recon_label, label, z, mu, logvar, batch_size, n_samples):
