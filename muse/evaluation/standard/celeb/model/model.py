from muse.evaluation.standard.celeb.model.model_components import *
...
class MUSE(nn.Module):
    def __init__(self, top_latents, img_latents, label_latents, use_cuda=False):
        ...
        # MNIST
        self.img_vae = VAE(latent_dim=img_latents, mod='celeb', use_cuda=use_cuda)
        self.label_vae = VAE(latent_dim=label_latents, mod='label_celeb', use_cuda=use_cuda)

        # Top Representation
        self.img_top_encoder = TopEncoder(input_dim=img_latents, latent_dim=top_latents)
        self.label_top_encoder = TopEncoder(input_dim=self.label_latents, latent_dim=top_latents)

        self.img_top_decoder = TopDecoder(latent_dim=top_latents, out_dim=img_latents)
    ...
    def infer(self, z_img=None, z_label=None):

        batch_size = z_img.size(0) if z_img is not None else z_label.size(0)
        ...
        if z_img is not None:
            img_mu, img_logvar = self.img_top_encoder(z_img)
    ...
    def generate(self, x_img=None, x_label=None):
        with torch.no_grad():

            # Encode MNIST Modality Data
            if x_img is not None:
                img_out, _, img_mu, _ = self.img_vae(x_img)
            else:
                img_out, img_mu = None, None
            ...
            top_mu, top_logvar = self.infer(z_img=img_mu, z_label=label_mu)
            top_z = self.reparametrize(top_mu, top_logvar)

            # Decode top representation
            img_z = self.img_top_decoder(top_z)
            img_top_out = self.img_vae.decoder(img_z)

            label_z = self.label_top_decoder(top_z)
            label_top_out = self.label_vae.decoder(label_z)

        return [img_out, img_top_out], [label_out, label_top_out]

    def encode_latent(self, x_img=None, x_label=None):
        with torch.no_grad():

            # Encode MNIST Modality Data
            if x_img is not None:
                _, _, img_mu, _ = self.img_vae(x_img)
            else:
                img_mu = None
    ...
    def forward(self, x_img, x_label):
        ...
        img_out, img_z, img_mu, img_logvar = self.img_vae(x_img)
        label_out, label_z, label_mu, label_logvar = self.label_vae(x_label)

        # Infer top representation
        top_mu, top_logvar = self.infer(z_img=img_z.clone().detach(), z_label=label_z.clone().detach())
        top_z = self.reparametrize(mu=top_mu, logvar=top_logvar)

        # FPA top representation
        fpa_mu_img, fpa_logvar_img = self.infer(z_img=img_z.clone().detach(), z_label=None)
        fpa_mu_lbl, fpa_logvar_lbl = self.infer(z_img=None, z_label=label_z.clone().detach())

        # Decode information from top
        img_top_z = self.img_top_decoder(top_z)
        label_top_z = self.label_top_decoder(top_z)

        return img_out, label_out, \
               [img_mu, img_logvar, img_z, img_top_z], \
...
class VAE(nn.Module):
    def __init__(self, latent_dim, mod, use_cuda=False):
        ...
        if self.mod == 'mnist':
            ...
            self.encoder = CelebEncoder(latent_dim=latent_dim)

            self.decoder = CelebDecoder(latent_dim=latent_dim)
        ...
        elif self.mod == 'label_celeb':
            self.encoder = AttributeEncoder(latent_dim=latent_dim)

            self.decoder = AttributeDecoder(latent_dim=latent_dim)
