...
from muse.evaluation.standard.mnist.model.trainer import Trainer
    ...
    def _record_artifacts(self, info, _config):
        ...
        if epoch % artifact_storage_interval == 0:
            ...
            with torch.no_grad():
                ...
                mnist_data = test_data[0]
                label_data = torch.nn.functional.one_hot(test_data[1], num_classes=10).float()
                ...
                # Generate modalities
                mnist_out, label_out  = self.model.generate(x_mnist=mnist_data,x_label=label_data)
                mnist_out, mnist_top_out = mnist_out[0], mnist_out[1]
                label_out, label_top_out = label_out[0], label_out[1]

                mnist_top_mnist_out, mnist_top_label_out = self.model.generate(x_mnist=mnist_data)
                mnist_top_mnist_out = mnist_top_mnist_out[1]
                mnist_top_label_out = mnist_top_label_out[1]

                label_top_mnist_out, label_top_label_out = self.model.generate(x_label=label_data)
                label_top_mnist_out = label_top_mnist_out[1]
                label_top_label_out = label_top_label_out[1]

                # Vision Recon
                img_comp = torch.cat([mnist_data.view(-1, 1, 28, 28).cpu(), mnist_out.view(-1, 1, 28, 28).cpu()])
                all_top_img_comp = torch.cat([mnist_data.view(-1, 1, 28, 28).cpu(), mnist_top_out.view(-1, 1, 28, 28).cpu()])
                img_top_img_comp = torch.cat([mnist_data.view(-1, 1, 28, 28).cpu(), mnist_top_mnist_out.view(-1, 1, 28, 28).cpu()])
                sym_top_img_comp = torch.cat([mnist_data.view(-1, 1, 28, 28).cpu(), label_top_mnist_out.view(-1, 1, 28, 28).cpu()])

                # Text Recon
                sym_res = np.argmax(F.log_softmax(label_out, dim=-1).cpu().numpy(), axis=1).tolist()
                sym_res_str = ''
                for i, item in enumerate(sym_res):
                    sym_res_str += str(item) + " "

                all_sym_res = np.argmax(F.log_softmax(label_top_out, dim=-1).cpu().numpy(), axis=1).tolist()
                all_sym_res_str = ''
                for i, item in enumerate(all_sym_res):
                    all_sym_res_str += str(item) + " "

                img_nx_sym_out_res = np.argmax(F.log_softmax(mnist_top_label_out, dim=-1).cpu().numpy(), axis=1).tolist()
                img_nx_sym_out_res_str = ''
                for i, item in enumerate(img_nx_sym_out_res):
                    img_nx_sym_out_res_str += str(item) + " "

                sym_nx_sym_out_res = np.argmax(F.log_softmax(label_top_label_out, dim=-1).cpu().numpy(), axis=1).tolist()
            ...
            with open(os.path.join(results_dir, 'all_top_label_res_str_e' + str(epoch) + '.txt'), "w") as text_file:
                print(all_sym_res_str, file=text_file)
            ex.add_artifact(os.path.join(results_dir, "all_top_label_res_str_e" + str(epoch) + '.txt'),
                            name="all_top_label_recon_e" + str(epoch) + '.txt')

            with open(os.path.join(results_dir,'img_top_label_out_res_str_e' + str(epoch) + '.txt'), "w") as text_file:
                print(img_nx_sym_out_res_str, file=text_file)
            ex.add_artifact(os.path.join(results_dir, "img_top_label_out_res_str_e" + str(epoch) + '.txt'),
                            name= "image_top_label_recon_e" + str(epoch) + '.txt')

            with open(os.path.join(results_dir,'label_top_label_out_res_str_e' + str(epoch) + '.txt'), "w") as text_file:
                print(sym_nx_sym_out_res_str, file=text_file)
            ex.add_artifact(os.path.join(results_dir, "label_top_label_out_res_str_e" + str(epoch) + '.txt'),
                            name= "label_top_label_recon_e" + str(epoch) + '.txt')
