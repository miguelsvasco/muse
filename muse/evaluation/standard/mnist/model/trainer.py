...
class Trainer(object):
    ...
    def loss_function(self, mnist_data, label_data, mnist_out, label_out, mnist_dist, label_dist, mu, logvar, fpa_mu, fpa_logvar):
        ...
        # Total loss
        loss = torch.mean(mnist_recon + label_recon + mnist_prior + label_prior + mnist_top_recon + label_top_recon + top_prior + top_fpa)

        return loss, mnist_recon, label_recon, mnist_prior, label_prior, mnist_top_recon, label_top_recon, top_prior, top_fpa
    ...
    def _run(self, train, epoch, dataloader, cuda):
        ...
        for batch_idx, modality_data in enumerate(dataloader):
            ...
            mnist_data = modality_data[0]
            label_data = torch.nn.functional.one_hot(modality_data[1], num_classes=10).float()
            ...
            # Forward
            mnist_out, label_out, \
            mnist_dist, label_dist, \
            top_mu, top_logvar, fpa_mu, fpa_logvar = self.model(mnist_data, label_data)

            # Losses
            loss, mnist_recon, label_recon, mnist_prior, label_prior,\
            mnist_top_recon, label_top_recon,\
            top_prior, top_fpa = self.loss_function(mnist_data, label_data, mnist_out, label_out, mnist_dist, label_dist,
