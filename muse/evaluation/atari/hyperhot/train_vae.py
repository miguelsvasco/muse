...
from muse.evaluation.atari.hyperhot.vae.trainer import VAETrainer
from muse.scenarios.multimodal_atari.hyperhot_dataset import HyperhotDataset

ex = sacred.Experiment(
    'hyperhot_muse_train_vae',
    ingredients=[
        ingredients.hyperhot_ingredient, ingredients.vae_ingredient,
...
def make_dataloaders(train_dataset_samples, test_dataset_samples, n_stack,
                     seed, n_enemies, pacifist_mode, sound_receivers,
                     batch_size):
    ...
    ds_train, ds_test = torch.utils.data.random_split(
        ds, [train_dataset_samples, test_dataset_samples])
...
class PostEpochCb(object):
    ...
    def _record_artifacts_maybe(self, info, _config):
        ...
        if epoch % artifact_storage_interval == 0:
            ...
            with torch.no_grad():
                ...
                # Vision Recon
                img_comp = torch.cat([image.view(-1, 2*80, 80).cpu(), img_recon.view(-1, 2*80, 80).cpu()], dim=0).unsqueeze(1)
                img_img_comp = torch.cat([image.view(-1, 2*80, 80).cpu(), img_img_recon.view(-1, 2*80, 80).cpu()], dim=0).unsqueeze(1)
                snd_img_comp = torch.cat([image.view(-1, 2*80, 80).cpu(), snd_img_recon.view(-1, 2*80, 80).cpu()], dim=0).unsqueeze(1)
    ...
    def _record_checkpoint(self, info, _config):
        ...
        pickleable_vae_config = sacred_copy(_config['vae'])
        env_config = dict(_config['hyperhot'])
        env_config['name'] = 'hyperhot'
        env_config['sound_normalization'] = \
            self.train_dataloader.dataset.dataset.sound_normalization()
...
def train(_config, _run):
    ...
    # Create model
    model = vae_utils.make_multimodalvae(vae_config, hyperhot_config)
    ...
    train_loader, test_loader = make_dataloaders(
        hyperhot_config['train_dataset_samples'],
        hyperhot_config['test_dataset_samples'], hyperhot_config['n_stack'],
        _config['seed'], hyperhot_config['n_enemies'],
        hyperhot_config['pacifist_mode'], hyperhot_config['sound_receivers'],
